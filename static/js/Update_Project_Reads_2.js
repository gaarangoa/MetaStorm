...
upload_dir="/home/raid/www/MetaStorm/main/Files/PROJECTS/"
  ...
  $.ajax({
      ...
      success: function(x) {
        ...
        $("#updatePname").html("<b>Name: </b>"+x.data[0]['project_name'])
      }
  });
  
  
  
  var logs=function(date, sid, pip, status){
    if (status=="done") {
        color='blue'
    }
  ...
  $.ajax({
      ...
      type: "POST",
      async: true,
      data: JSON.stringify({sql:'select * from jobs where pid="'+pid+'" and pip="'+pip+'" and uid="'+uid+'" order by strftime(date) '}),
      contentType: "application/json; charset=utf-8",
      success: function(x) {
        //console.log(x.data)
        x.data.forEach(function(item, index){
          $("#logs").append(logs(item.date, item.sid, item.pip, item.status))
          })
 ...
 $.ajax({
     ...
     type: "POST",
     async: true,
     data: JSON.stringify({sql:'select reference_id, reference_name, reference_description, status, user_id from reference'}),
     contentType: "application/json; charset=utf-8",
     success: function(x) {
       ////console.log(x)
       ...
       $("#jqxNavigationBar").jqxNavigationBar({ width: "100%", height: "auto", expandMode: 'toggle', expandMode: 'multiple', expandedIndexes: [0, 1]});
...
/*Load my (user) reference databases*/
$.ajax({
    url: machine+"consult",
    type: "POST",
    async: true,
    ...
    success: function(x) {
      ...
      for(i=0; i<x.data.length;i++){
        if(x.data[i]['status']=="done"){
          $("#myDatasets").append('<input type="checkbox" name="selectedReferenceDatasets" value="'+x.data[i]['reference_id']+'" > '+x.data[i]['reference_name']+'<br>')
          REFS[x.data[i]['reference_id']]=x.data[i]['reference_name']
        }
      }
...
$.ajax({
    ...
    success: function(x) {
      ...
      for(i=0; i<x.data.length;i++){
        if(x.data[i]['status']=="done"){
          $("#OthersDatasets").append('<input type="checkbox" name="selectedReferenceDatasets" value="'+x.data[i]['reference_id']+'" > '+x.data[i]['reference_name']+'<br>')
          REFS[x.data[i]['reference_id']]=x.data[i]['reference_name']
        }
      }
  ...
  var get_samples_info=function(){
      ...
      $.ajax({
          ...
          success: function(x) {
            ...
            for(i=0; i<x.data.length; i++){
              item={ID:x.data[i]['c.sample_id'],Name:x.data[i]['c.sample_name'],Condition:x.data[i]['c.sample_set'],Environment:x.data[i]['c.environment'],Library:x.data[i]['c.library_preparation'],Mate1:x.data[i]['c.reads1'],Mate2:x.data[i]['c.reads2']}
              data.push(item)
            }
...
$(document).on("click","#RemoveSample", function(){
    ...
    $.ajax({
        ...
        type: "POST",
        async: true,
        data: JSON.stringify({sid:sid,pid:pid,pip:pip, uid:uid}),
        contentType: "application/json; charset=utf-8",
        success: function(dat) {
            //console.log(dat)
...
/*Insert new sample*/
  ...
  document.getElementById("NumberSamplesButton").onclick = function() {setupNumberSamples(); $("#submitMetagenome").removeAttr('disabled')};
  $("#NumberSamples").on('keydown', function(e) {if(e.keyCode == 13){setupNumberSamples(); $("#submitMetagenome").removeAttr('disabled')}});
  ...
  var setupNumberSamples = function(){
    ...
    matrix=[['','', '', '', '0', '0']]//[['Sample Name','Sequence Method', 'Biome', 'Experiment Type', 'Latitude', 'Longitude']]
  ...
  var retrieve_hst = function(x){
    ...
    for (i=0; i<totalsamples; i++){
      e=$(x[i]).contents()
      xsm=$(e[2]).text(); xsm=xsm.slice(0, xsm.length-1)
      xbi=$(e[3]).text(); xbi=xbi.slice(0, xbi.length-1)
      xex=$(e[4]).text(); xex=xex.slice(0, xex.length-1)
...
// inser samples information into the sql database
  $(document).on("click","#submitMetagenome", function(){
      ...
      $.ajax({
          ...
          type: "POST",
          data: JSON.stringify({samples: samples, pid:pid, uid:uid, pip:pip}),
          async: true,
          contentType: "application/json; charset=utf-8",
...
//// share project
  ...
  $(document).on("click","#ShareProject", function(){
    ...
    $.ajax({
        ...
        type: "POST",
        async: true,
        data: JSON.stringify({users:shared_users, pid:pid}),
        contentType: "application/json; charset=utf-8",
        success: function(x) {
...
var submit_analysis =  function(sid, read1, read2, sname){
    ...
    if(rids.length==0){
            ...
            rid_names="<ul>"+get_checkbox("selectedReferenceDatasets").map(function(e,i,a){return("<li><p>"+REFS[a[i]]+"</p></li>")}).join("")+"</ul>"

            msg='Dear MetaStorm user, <br><br>The sample <b>'+sname+'</b> has been queued into the <b>MetaStorm</b> server. The time for making the analysis depends on the current web traffic and availability of the server. You can check the run status on MetaStorm. Also, you will receive an email once the analisis is done.  Please make sure to check your spam folder <br><br><h3>Analysis</h3> You are currently running the </b>'+pip+' Pipeline</b> with the following reference databases'+rid_names+"<br><br>Thank you<br>MetaStorm</b> Team"


            $("#runstatusbar").html(wait_bar(50))

            datasent={uid:uid, pid:pid, sid:sid, read1:read1, read2:read2, pip:pip, rids:rids}
            ...
            $.ajax({
                ...
                success: function(dat) {
                  ...
                  $("#RunMetaGen").disabled = false;
                   $("#runstatusbar").html("")
                   send_email(uid,msg,'Processing sample: '+sname+" ("+sid+")")
                  //get_samples_info()
  ...
  var get_run_table=function(){
      ...
      $.ajax({
          ...
          success: function(x) {
            ...
            //alert(x.data[0])
              ...
              remove_sample =  function(arg){
                ...
                $.ajax({
                    ...
                    type: "POST",
                    async: true,
                    data: JSON.stringify({sid:arg[1],pid:arg[3],pip:arg[2], uid:userID}),
                    contentType: "application/json; charset=utf-8",
                    success: function(dat) {
                      $("#tab_runo_box_overlay").remove()
                        //console.log(dat)
              ...
              var runHTML = function (instance, td, row, col, prop, value, cellProperties) {
                ...
                value=value.split(':')
                if(value[1]=='done'){avl=''; type='primary'}else if(value[1]=='running' || value[1]=='queue'){avl='disabled="true"'; type='primary'}else{avl=''; type='primary'}
                if (value[1] == 'null') {
                    avl=''; type='primary'
                }
            ...
            for(i=0; i<x.data.length; i++){
              item={ID:x.data[i]['sample_id']+":"+x.data[i]['status'],Sample:x.data[i]['sample_name'],Mate1:x.data[i]['reads1'],Mate2:x.data[i]['reads2'], run:0}
              data.push(item)
            }
