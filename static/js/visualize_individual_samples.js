...
function individual_samples(data){
    ...
    // get the data
    val2=[['sample_id','gene_id','gene_counts','gene_16S_normalization','gene_RPKM_normalization','gene_functional_category_id','gene_functional_category','description']]
    val=[]
    ...
    var ndx = crossfilter(val),
        CategoryDim  = ndx.dimension(function(d) {return d.category_id;}),
        GeneDim  = ndx.dimension(function(d) {return d.gene_id;}),
        
        FunctionalCategoryCounts = CategoryDim.group().reduceSum(function(d) {return +d.gene_counts;}),
        GeneCounts = GeneDim.group().reduceSum(function(d) {return d.gene_counts/d.num_categories_in_gene;}),
    ...
    $(document).on("click","#download_one_sample_table_taxo", function(e){   
            ...
            if(pip="matches"){
                window.location.href = machine_http+'download/'+Base64.encode("Files/PROJECTS/"+pid+"/matches/"+sid+'/'+"alignment."+rid+".matches.taxonomy.abundance.json");
            }else{
                // window.location.href = machine_http+'download/'+Base64.encode("Files/PROJECTS/"+pid+"/matches/"+sid+'/'+"alignment."+rid+".matches.taxonomy.abundance.json");
                window.location.href = machine_http+'download/'+Base64.encode("Files/PROJECTS/"+pid+"/assembly/idba_ud/"+'/'+"alignment."+rid+".matches.taxonomy.abundance.genes.json");
            }
