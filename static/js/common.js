...
logout = function(uid){
   $.ajax({
        ...
        success: function(x) {window.open(machine+"login","_self")}
    });
}
...
fnt= function(name){ 
      if(/unknown/.test(name)){
                 ...
                 if (out=="") {return 'unknown'}else{return out}
              }
}
...
var ptree=function(treeData, domain,tag,clpd,names, Pcolor, width, height, rsize, condition, pip,rid,offset, stk){
  ...
  var margin = {top: 10, right: 5, bottom: 10, left: 20}
  ...
  function update(source) {
    ...
    nodeUpdate.select("circle")
      .attr("r", function(d){return rsize}) //!FIXME EDIT HERE!!!
      .style("fill", function(d) { return d._children ? color(d[Pcolor]) : color(d[Pcolor]); });
...
send_email = function(uid,msg,sub){
   ...
   $.ajax({
      ...
      type: "POST",
      data: JSON.stringify({uid:uid,msg:msg,sub:sub}),
      async: true,
      contentType: "application/json; charset=utf-8",
...
clustergram = function(target,heatmap, width){
  window.inchlib = new InCHlib({ //instantiate InCHlib
    ...
    inchlib.add_color_scale("mine", {"start": {"r":0, "g": 200, "b": 0}, "middle": {"r": 0, "g": 0, "b": 0}, "end": {"r": 255, "g": 0, "b": 0} });
...
process_stacked = function(data){
    ...
    for(i=1;i<data.length;i++){
        d.push({name:fnt(data[i][0]),data:data[i].splice(1)})
    }
    return([categories,d])
}
...
process_stacked_from_tree = function(data){
    ...
    for(i=1;i<data.length;i++){
      key=fnt(data[i][0])
      d[key]={name:fnt(data[i][0]),data:data[i].splice(1)}
    }
...
loading_status_sql = function(pid, filename, status){
    ...
    $.ajax({
        ...
        type: "POST",
        async: true,
        data: JSON.stringify({sql:Base64.encode(sql)}),
        contentType: "application/json; charset=utf-8",
        success: function(x) {
          ////console.log(x)
        }
...
function upload_php(obj,sid,pid,pip,uid,upload_dir){
		...
		url : 'http://bench.cs.vt.edu/plupload/examples/upload.php',
		...
		filters : {
			...
			mime_types: [
				{title : "Raw NGS files", extensions : "fastq,gz,fasta,fa,fq"},
				{title : "Compressed NGS files", extensions : "gz"}
			]
		},
...
var draw_table=function(div, data){
   ...
   var rowOrder = function(data){
      ...
      Object.keys(data[0]).forEach(function(element, index, array){
         ...
         if(element!='Long_Description' && element!="description" && element!="samples") {x.push({data:element})}
         })
      x.push({data:"description"})
      x.push({data:"Long_Description"})
      return(x)
   }
   ...
   var colWidth = function(data){
      ...
      Object.keys(data[0]).forEach(function(element, index, array){
         //return d.children || d._children ? "end" : "start";
         if(element!='Long_Description' && element!="description" && element!="samples") {x.push(50)}
         })
      x.push(200)
      x.push(300)
      return(x)
   ...
   $("#"+div).html('')
      var container = document.getElementById(div), previous_samples;
