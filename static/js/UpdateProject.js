...
upload_dir="/home/raid/www/MetaStorm/main/Files/PROJECTS/"
  ...
  $.ajax({
      ...
      type: "POST",
      async: true,
      data: JSON.stringify({sql:'select * from (select * from (select * from jobs where pid="'+pid+'" and pip="'+pip+
...
var submit_analysis =  function(sid, read1, read2, sname){
    ...
    if(read1 == "" || read2 == ""){alert('Error: Raw reads file (*.fastq.gz) is missing')}
    else{
    ...
    if(rids.length==0){
            ...
            datasent={uid:uid, pid:pid, sid:sid, read1:read1, read2:read2, pip:pip, rids:rids, msg:msg}
