...
machine="/MetaStorm/"
  ...
  $.urlParam = function(name){
      ...
      $("#"+div).jqxDataTable(
            {
                ...
                filterable: true,
                columns: [
                  { text: '<b>Biome</b>', dataField: 'biome', width: "15%", align: 'center', cellsAlign: 'center'},
                  { text: '<b>Project Name</b>', dataField: 'project_name', width: "20%", align: 'center', cellsAlign: 'left',
                  ...
                  { text: '<b>Description</b>', dataField: 'project_description', width: "35%", align: 'center', cellsAlign: 'left' },
                  { text: '<b>Analysis</b>', dataField: 'pipeline', width: "20%", align: 'center', cellsAlign: 'center',
                     cellsRenderer: function (row, column, value, rowData) {
                        ...
                        if (rowData.access=='unlock') {
                           ...
                           pipsi=value.split(",").forEach(function(a,b,c){
                              pips += '<a target="_blank" href="/MetaStorm/ExploreProject?uid='+userID+'&pid='+rowData.project_id+'&pip='+a+'">'+a+'</a> '
                              })
                           return pips
                        }
                        else{
                  ...
                  { text: '<b>Samples</b>', dataField: 'samples', width: "10%", cellsAlign: 'center', align: 'center' },
   ...
   $.ajax({
       ...
       pron=0
       dat.data.forEach(function(a,b,c){return (pron +=a.samples)})
   ...
   button_selected_projects =  function(div){
      ...
      var values = $(div+" option").map(function() { return $(this).val(); });
      values=$("#projects_compare_list option")
      
      vv=[]; vn=[];
      for(i=0;i<values.length;i++){vv.push($(values[i]).val()); vn.push($(values[i]).text())}
      ...
      if (vv.length==0) {
        alert('Select at least one project')
         ...
         //console.log(vv)
         var x=window.open("http://bench.cs.vt.edu/MetaStorm/compare_projects?uid="+userID+"?pip=matches");
             x.pids=vv
             x.pnames=vn
