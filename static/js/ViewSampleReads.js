...
// get the databases for taxonomy that you have used to run
    ...
    var breakdown=function(dataset){
      sent={uid:uid, pid:pid, sid:sid, rid:dataset, pip:'matches'}
...
$(document).on("click","#VisualizeDBR", function(){
    ...
    $.ajax({
        ...
        type: "POST",
        async: true,
        data: JSON.stringify({uid:uid, pid:pid, sid:sid, pip:"assembly", value:"rpkm", rid:rid}),
        contentType: "application/json; charset=utf-8",
        success: function(x) {
          console.log(x)
...
$(document).on("click","#loadFunction", function(){
    ...
    sent={uid:uid, pid:pid, sid:sid, pip:"assembly", rid:rid}
    console.log(sent)
    $.ajax({
...
var set_taxonomy_small_pie_chart=function(lid){
  ...
  $.ajax({
      ...
      type: "POST",
      async: true,
      data: JSON.stringify({uid:uid, pid:pid, sid:sid, tid:"none", lid:lid, pip:"assembly", rid:rid}),
      contentType: "application/json; charset=utf-8",
      success: function(x) {
        console.log(x)
...
// By default it loads the silva database
  ...
  $.ajax({
      ...
      type: "POST",
      async: true,
      data: JSON.stringify({uid:uid, pid:pid, sid:sid, pip:"assembly", value:"rpkm", rid:"abcdefghij"}),
      contentType: "application/json; charset=utf-8",
      success: function(x) {
        //console.log(x)
...
type: "POST",
async: true,
data: JSON.stringify({uid:uid, pid:pid, sid:sid, pip:"assembly"}),
contentType: "application/json; charset=utf-8",
success: function(x) {
    //console.log(x)
