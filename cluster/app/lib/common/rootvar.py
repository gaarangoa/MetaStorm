...
from app.lib.common.module import load_features, load_features2, load_features3
...
# BASIC CONSTANTS
#******************************************************************************
__root_dir__="/groups/metastorm_cscee/MetaStorm/"
...
class program:
    def __init__(self,program,sample,db):
        if program=='bowtie2':
            ...
            self.options="--fast-local --no-unal -p "+p
            ...
            self.cmd=" ".join([self.exe,self.options, sample.reads1.replace(".gz",""), sample.reads2.replace(".gz",""), self.out])
            mkdir(self.path)
            ...
            self.options="-evalue 1e-5  -outfmt 6 -max_target_seqs 1 -perc_identity 20 -num_threads  "+p+"  "
            self.sample=sample
            self.db=db
            self.program=program
            self.path=sample.assemblyDir+"/idba_ud/"+sample.id+"/"
            ...
            self.options="--evalue 1e-5 --id 20 -k 1 -p "+p+" "
            self.sample=sample
            self.db=db
            self.path=sample.assemblyDir+"/idba_ud/"+sample.id+"/"
            self.output=self.path+"/pred.genes"
            ...
            self.prot=self.output+'.prot.fa'
            self.out=self.output+"."+db.name+".matches"
            self.cmd=" ".join([self.exe, self.nucl,"--input_type fasta " ,' --nproc ', p," --mpa_pkl", self.root+"db_v20/mpa_v20_m200.pkl", "--bowtie2db", self.root+"db_v20/mpa_v20_m200","-t rel_ab_w_read_stats", "--sample_id_key",sample.id, "-o", self.out, ' --biom', self.out+".biom", "--bowtie2_exe", __ROOTEXEDIR__+"/bowtie2"])
            ...
            self.out=self.output+"."+db.name+".matches"
            self.cmd=" ".join([self.exe, sample.reads1+","+sample.reads2," --bowtie2out ",self.out+".sam" ,"--input_type fastq ",' --nproc ', p," --mpa_pkl", self.root+"db_v20/mpa_v20_m200.pkl", "--bowtie2db", self.root+"db_v20/mpa_v20_m200","-t rel_ab_w_read_stats", "--sample_id_key",sample.id, "-o", self.out, ' --biom', self.out+".biom", "--bowtie2_exe", __ROOTEXEDIR__+"/bowtie2"])
    def run(self):
...
class dataset:
    def __init__(self,dataset):
        ...
        self.len=load_features3(self.dir+"dataset.len")
        self.func=load_features2(self.dir+"dataset.func")
...
class samples:
    def __init__(self,x,path="/"):
        ...
        self.library=x[5]
        self.reads1=__ROOTPRO__+"/"+self.pid+"/READS/"+x[6].replace('.gz',"")
        self.reads2=__ROOTPRO__+"/"+self.pid+"/READS/"+x[7].replace('.gz',"")
        self.ureads=__ROOTPRO__+"/"+self.pid+"/matches/"+x[0]+"/flash.extendedFrags.fastq"
        self.tree=__ROOTPRO__+"/"+self.pid+"/matches/"+x[0]+"/tree.json"
        self.atree=__ROOTPRO__+"/"+self.pid+"/assembly/"+x[0]+"/tree.json"
        self.path=__ROOTPRO__+"/"+self.pid
        self.assemblyDir=__ROOTPRO__+"/"+self.pid+"/assembly/"
        self.matchesDir=__ROOTPRO__+"/"+self.pid+"/matches/"+x[0]+"/"
