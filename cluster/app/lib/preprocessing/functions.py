...
class trimmomatic:
	def __init__(self,m1,m2,outdir,sample_id):
		...
		self.options="PE -phred33 -threads "+rootvar.p+" "
		self.outd=outdir+"/"
		self.om1 = self.mate1+'.fq'
		self.om2 = self.mate2+'.fq'
		self.cmd =" ".join([self.exe, self.options, self.mate1, self.mate2, self.om1, self.mate1+".unpaired.fq.gz", self.om2, self.mate2+".unpaired.fq.gz", "ILLUMINACLIP:TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36 CROP:128 > ", self.outd+self.sid+'trim.log', "2>&1"])
	def run(self):
    		print 'trimmomatic run: '+self.cmd
		mkdir(self.outd)
		try:
			os.system(self.cmd)
