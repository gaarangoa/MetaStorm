...
def assembly(file):
    ...
    return {'reads':int(reads),'alreads':aligned_reads,'avgreads':int(avgreads), 'scaffolds':int(sca[1]), 'n50':int(sca[3]), 'maxScaff':int(sca[5]), 'avgScaff':int(sca[7]), 'totalScaffLen':int(sca[-1]), 'NumGenes':len(ldist), 'maxGeneLength':max(ldist), 'avgGene':int(np.mean(ldist))}
