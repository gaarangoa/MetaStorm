...
def csession(request):
    try:
        ...
        session_info = query_db('select * from session where uid="'+uid +
                                '" and ip="'+ip+'" and browser="'+browser+'" and status="online"')[0]
        return True
...
def lgn():
    ...
    credentials = query_db('select * from user where user_affiliation="' +
                           str(email)+'" and user_password="'+str(password)+'"')
    # return jsonify(status=credentials)
    if not credentials:
        return jsonify(status='fatal')
    elif not credentials[0]['user_password'] == password:
        return jsonify(status='fatal')
    g.db.execute("INSERT or REPLACE into session VALUES (?,?,?,?)",
                 (credentials[0]['user_id'], userIP, browser, 'online'))
    g.db.commit()
...
def change_password():
    try:
        ...
        cpass = query_db(
            'select user_password from user where user_id="'+str(uid)+'"')
        if cpass[0]['user_password'] == oldp1:
            g.db.execute('UPDATE user SET user_password="' +
                         new1+'" WHERE user_id="'+uid+'"')
            g.db.commit()
...
def forgot_pass():
    try:
        ...
        credentials = query_db(
            'select * from user where user_affiliation="'+str(gemail)+'"')
        email.send_email(credentials[0]['user_name'], gemail, 'MetaStorm request',
                         'Dear MetaStorm user, <br> This is your password <br><br><br>' + credentials[0]['user_password']+'<br><br>MetaStorm Team')
...
def insert_new_user():
    ...
    try:
        g.db.execute('INSERT INTO user (user_id,user_password,user_name,user_affiliation,organization,country,date) VALUES (?,?,?,?,?,?,?)',
                     (id, str(password), str(name), str(eMail), str(organization), str(country), date))
        g.db.commit()
        msg = "Dear MetaStorm user, <br> You have been granted with access to our web service. Please login at <a href='bench.cs.vt.edu/MetaStorm/login'>MetaStorm</a> in order to use the web resource <br> User - " + \
            eMail+"<br> Password - "+password+"<br><br><b>MetaStorm</b> team"
        subject = "MetaStorm - New account requested"
        #msg="we are done"
        g.db.execute(
            'INSERT INTO user_projects (user_id,project_id) VALUES(?,?)', (id, 'pNAr1xdrsO6H7xs'))
        g.db.commit()
...
def insert_project():
    try:
        ...
        id = rootvar.get_alphabet(15)
        resu = g.db.execute("INSERT INTO project VALUES (?,?,?,?,?) ",
                            (id, name, "lock", description, rootvar.__ROOTPRO__+"/"+id))
        g.db.commit()
        resu = g.db.execute(
            "INSERT INTO user_projects VALUES (?,?)", (userID, id))
        g.db.commit()
...
def remove_element():
    try:
        ...
        if uid == "TesREPDooc73Ohw":
            return 'User not allowed'
        dir_to_sid = query_db(
            ' select * from project where project_id="'+pid+'"')[0]['project_path']
        g.db.execute('delete from user_projects where user_id="' +
                     uid+'" and project_id="'+pid+'"')
    ...
    try:
        ...
        for sample in samples:
            if sample != []:
                ...
                sdata.append(item)
                g.db.execute(
                    "INSERT INTO samples VALUES (?,?,?,?,?,?,?,?,?,?)", (item[:-2]+("", "")+item[-2:]))
                g.db.commit()
...
def remove_sample():
    try:
        # {'pid':'bcrxuradwctalvl', 'sid':'sid', 'pip':'pip'}#
        data = request.get_json()
        pid = str(data['pid'])
        sid = str(data['sid'])
        pip = str(data['pip'])
        ...
        if uid == "TesREPDooc73Ohw":
            return 'User not allowed'
        g.db.execute('delete from samples where project_id="' +
                     pid+'" and sample_id="'+sid+'"')
        g.db.execute('delete from project_status where project_id="' +
                     pid+'" and sample_id="'+sid+'"')
        g.db.commit()
        if pip == "assembly":
            dir_to_sid = query_db(' select * from project where project_id="'+pid+'"')[
                0]['project_path']+"/"+pip+"/idba_ud/"+sid+"/"
        else:
            dir_to_sid = query_db(' select * from project where project_id="' +
                                  pid+'"')[0]['project_path']+"/"+pip+"/"+sid+"/"
        os.system('rm -r '+dir_to_sid)
        return dir_to_sid
    except Exception as inst:
        return "ERROR: "+str(inst)
...
def load_file(file, projectid):
    try:
        app.config['UPLOAD_FOLDER'] = rootvar.__ROOTPRO__ + \
            "/"+projectid+"/READS/"
        if not os.path.exists(rootvar.__ROOTPRO__+"/"+projectid+"/READS/"):
            os.makedirs(rootvar.__ROOTPRO__+"/"+projectid+"/READS/")
        if file:
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        g.db.execute("INSERT or REPLACE INTO fastqFiles VALUES (?,?,?,?)",
                     (projectid+'_'+file.filename, projectid, file.filename, 'done'))
        g.db.commit()
    except Exception as inst:
        g.db.execute("INSERT or REPLACE INTO fastqFiles VALUES (?,?,?,?)",
                     (projectid+'_'+file.filename, projectid, file.filename, inst))
        g.db.commit()
...
def process_up_ref_dataset():
    try:
        # {'taxofile':'none', 'functfile':'DATASET.func', 'rid':'eyzbhakkpq', 'seqfile':'dataset.fa', 'rname':'card_sample', 'uid':'ihqvnteormayzoy'}#
        data = request.get_json()
        # x=sql.SQL(main_db)

        g.db.execute('UPDATE reference SET seqfile="'+data['seqfile']+'",  taxofile="' +
                     data['taxofile']+'", functfile="'+data['functfile']+'" WHERE reference_id="'+data['rid']+'"')
        g.db.commit()
        refdb = query_db(
            'SELECT * from reference where reference_id="'+data['rid']+'"')[0]
        # return jsonify(data=refdb)
        # return jsonify(data=data)
        if str(data["taxofile"]) != "none":
            updb.GetUniqueIdentifier(refdb['reference_path'], refdb['reference_format_path'],
                                     refdb['taxofile'], "taxo", g, data['rid'])  # for taxonomy
            glen.seqlen(refdb['reference_path']+'/'+refdb['seqfile'],
                        refdb['reference_format_path']+'/dataset.len')
            formatdb.main(refdb['reference_path'], refdb['reference_format_path'],
                          refdb['sequence_type'], refdb['seqfile'], g, data['rid'])
            g.db.execute(
                'UPDATE reference SET status="done" WHERE reference_id="'+data['rid']+'"')
        if str(data["functfile"]) != "none":
            updb.GetUniqueIdentifier(refdb['reference_path'], refdb['reference_format_path'],
                                     refdb['functfile'], "func", g, data['rid'])  # for function
            glen.seqlen(refdb['reference_path']+'/'+refdb['seqfile'],
                        refdb['reference_format_path']+'/dataset.len')
            formatdb.main(refdb['reference_path'], refdb['reference_format_path'],
                          refdb['sequence_type'], refdb['seqfile'], g, data['rid'])
            rootvar.tsv2json(
                refdb['reference_format_path']+'/dataset.description', "\t")
            g.db.execute(
                'UPDATE reference SET status="done" WHERE reference_id="'+data['rid']+'"')
        g.db.commit()
...
def RunMetaGen():
    try:
        ...
        x = email.send_email(
            S[0]['user_name'],
            S[0]['user_affiliation'],
            'Processing sample: ' + T[0]['sample_name'],
            "Dear MetaStorm User, <br><br><br> the sample <b>" +
            T[0]['sample_name'] +
...
def ShareProject():
    try:
        ...
        for uname in users:
            try:
                uid = query_db(
                    'select * from user where user_affiliation="'+uname+'"')[0]['user_id']
                g.db.execute(
                    'INSERT INTO user_projects VALUES (?,?)', (uid, pid))
            except:
                pass
        g.db.commit()
        return 'success'
...
def get_assembly_logs():
    try:
        ...
        if lid == "assembly":
            l1 = logassembly(rootvar.__ROOTPRO__+"/"+pid +
                             "/assembly/idba_ud/"+sid+"/")
        return jsonify(matrix=l1)
    except Exception as inst:
        return "ERROR: "+str(inst)
...
def format_matrix_ht(Q, description):
    ...
    for ix, i in enumerate(Q):
        data.setdefault("data", []).append(
            {str(header[jx]): j for jx, j in enumerate(i)})
    return data['data']
...
def get_all_samples_tree():
    try:
        ...
        try:
            db_desc = json.load(
                open(rootvar.__ROOTDBS__+"/"+data['rid']+"/dataset.description.json"))
        except:
            db_desc = False
        ...
        if X[0] == 'taxonomy':
            ...
            P = process_matrix_vis(N, ie, ib)
            Q = format_matrix_ht(format_matrix(
                np.transpose(P).tolist()), db_desc)

            for ix, i in enumerate(Q):
                try:
                    Q[ix]['description'] = db_desc[i['samples']
                                                   ]['X1'].replace('"', "")
                except:
                    Q[ix]['description'] = 'Function'
                try:
                    Q[ix]['Long_Description'] = db_desc[i['samples']
                                                        ]['X2'].replace('"', "")
                except:
...
def get_multiple_projects():
    try:
        ...
        for pid in data['pids']:
            ...
            if X[0] == 'taxonomy':
                matrix = [[pid+":"+i[0], i[1], i[2], i[3]]
                          for i in rootvar.get_matrix_level(data, data["lid"])]
            elif X[0] == 'function':
        ...
        #tmp.sort(key=lambda x: float(x[1]+x[2]) , reverse=True)
        heatmap = IL.main(m2js, None)
        heatmap[0]['data']['feature_names'] = order = [
            i.split(":")[1] for i in heatmap[0]['data']['feature_names']]
...
def sendEmail():
    try:
        ...
        subject = data['sub']
        S = query_db('select * from user where user_id="'+uid+'"')
        x = email.send_email(S[0]['user_name'], S[0]
                             ['user_affiliation'], subject, message)
        return "success"
    except Exception as inst:
        return "ERROR: "+str(inst)
...
from app.lib.retrieve.retrieve import get_results
