...
while 1:
    try:
        jobs = database.exe(
            "select * from jobs where priority='normal' and status!='done'")
        ...
        for job in jobs:
            ...
            status = os.popen('ssh gustavo1@newriver1.arc.vt.edu "cat ' +
                              fromf+'/arc_run.qsub.status "').read().split("\n")[0]
            print(json.dumps({"Pipeline": pip, "sampleID": sid, "ProjectID": data['pid'], "UserID": uid,
                              "Status": status, "from": fromf, "tof": tof, "jobid": jid, "sample": SAMPLE}, indent=4))
            ...
            if status == 'done':
                ...
                for refi in f2s:
                    for fi in refi:
                        print fi
                        os.system(
                            'scp gustavo1@newriver1.arc.vt.edu:/'+fromf+fi+" "+tof+fi)
        ...
        errda = json.dumps({"Pipeline": pip, "sampleID": sid,
                            "ProjectID": data['pid'], "UserID": uid, "Status": status, "from": fromf, "tof": tof}, indent=4)
        x = email.send_email('Gustavo Arango', 'gustavo1@vt.edu', 'MetaStorm Notification: ' +
                             sid, 'error in watch.py: '+str(inst) + '\nStatus: ' + status + "\n" + errda)
        time.sleep(120)
