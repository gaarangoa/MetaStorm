...
def taxonomy(data):
    ...
    log.debug([1, "sample_tree_file", all_samples_tree_file])
    ...
    for sid in sids:
        try:
            ...
            xpath = rootvar.__ROOTPRO__ + "/" + pid + "/"

            log.debug([2, "xpath_from_database", xpath])

            sample = rootvar.samples(samples[0], xpath)

            rf = rootvar.result_files(pid, analysis, pipeline, sample.id, rid)
            log.debug([3, "File with graph", rf.pk])

            view = rootvar.ViewSampleResults(sample, pipeline, rid, analysis, rf)
            log.debug([4, "Database File: ", view.file])

            temp_matrix = view.all()
            log.debug([5, "results after loading from db", temp_matrix])

            FULL_MATRIX += temp_matrix
            Gp = nx.read_gpickle(rf.pk)

            log.debug([6, "Loaded Graph -> Edges", len(Gp.edges())])
...
def functional(data):
    ...
    log = open(
        rootvar.__ROOTPRO__ + "/" + pid + "/" + pipeline + "/RESULTS/" + rid + ".log",
        "w",
    )
