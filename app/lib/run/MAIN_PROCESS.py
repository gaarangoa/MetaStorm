...
def process(projectid,sampleid,db,protocol,reads1, reads2, good_reads):
    ...
    if not db.taxo=="none":
        ...
        cmd=" ".join([root.__ROOTEXEDIR__+'bowtie2', '--very-fast-local -p '+p+' --no-unal --no-hd --no-sq -x', db.bowtie, '-1', sample.reads1, '-2', sample.reads2, '-S', sample.matchesDir+'/alignment.'+db.id+'.matches >>', root.log, '2>&1'])
    ...
    if not db.func=="none":
        ...
        if not root.isdir(sample.matchesDir+'/merged.reads.fastq'): os.system(cmd)
        #Get fasta files
        ...
        cmd=' '.join([root.__ROOTEXEDIR__+'/diamond blastx --id 60 -p '+p+' -k 1 -e 1e-5 -d', db.diamond, '-a', dout+'.pre', '-q', din, '>>', root.log, "2>&1"])
        ...
        if not root.isdir(sample.matchesDir+'/alignment.'+db.id+'.matches.function.abundance.results.sqlite3.db'):
            abundance=pb(dout+'.matches', db.func, db.len, db.funcdb, "function", db.name, fileso.GGenes+".rpkm", good_reads)
