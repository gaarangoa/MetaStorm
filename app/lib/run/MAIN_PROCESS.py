...
def process(projectid, sampleid, db, protocol, reads1, reads2, good_reads, sample):
    ...
    if not db.taxo == "none":
        ...
        cmd = " ".join([root.__ROOTEXEDIR__+'bowtie2', '--fast-local --no-discordant -p '+p+' --no-unal --no-hd --no-sq -x', db.bowtie,
                        '-1', sample.reads1, '-2', sample.reads2, '-S', sample.matchesDir+'/alignment.'+db.id+'.matches >>', root.log, '2>&1'])
        log.info("running: {}".format(cmd))

        if not root.isdir(sample.matchesDir+'/alignment.'+db.id+'.matches'):
            os.system(cmd)
        ...
        abundance = parse_sam(
            sample.matchesDir+'/alignment.'+db.id+'.matches', db, good_reads)
        G = txp.taxonomy_tree(abundance, sample.matchesDir +
                              '/alignment.'+db.id+'.matches', protocol, "taxonomy", db.id)
        abn = root.SampleResults(sample, G, protocol, db.id, "taxonomy", sample.matchesDir +
                                 '/alignment.'+db.id+'.matches')  # Store data in the sql TABLE
        abn.start()
        # update_status(x,sampleid,db.id,protocol,"Done")
        return 'success'
    if not db.func == "none":
        fileso = root.result_files(
            projectid, "function", protocol, sampleid, db.name)
        # Merge paired ends
        ...
        cmd = " ".join([root.__ROOTEXEDIR__+"fq2fa --merge ", sample.reads1,
                        sample.reads2, sample.matchesDir+"/merged.reads.fasta"])
        log.info("running: {}".format(cmd))
        ...
        cmd = ' '.join([root.__ROOTEXEDIR__+'/diamond blastx --id 60 -p '+p+' -k 10 -e 1e-5 -d',
                        db.diamond, '-a', dout+'.pre', '-q', din, '>>', root.log, "2>&1"])
        log.info("running: {}".format(cmd))

        if not root.isdir(dout+'.daa'):
            os.system(cmd)
        cmd = ' '.join([root.__ROOTEXEDIR__+'/diamond view -a', dout +
                        '.pre.daa', '-o', dout+'.matches -f tab', ">>", root.log, "2>&1"])
        log.info("running: {}".format(cmd))

        if not root.isdir(dout+'.matches'):
            os.system(cmd)
        ...
        abundance = pb(dout+'.matches', db.func, db.len, db.funcdb,
                       "function", db.name, fileso.GGenes+".rpkm", good_reads, 'matches')
        #abundance=pdx(dout+'.matches', db, good_reads)
        abn = root.SampleResults(
            sample, 'none', protocol, db.name, "function", dout+'.matches')
        abn.createFuncDb(abundance)
        # update_status(x,sampleid,db.id,protocol,"Done")
        #os.system('rm '+sample.matchesDir+'/merged.reads.fastq >> '+root.log+" 2>&1")
        os.system('rm '+sample.matchesDir +
                  '/merged.reads.fasta >> '+root.log+" 2>&1")
        return 'success'
